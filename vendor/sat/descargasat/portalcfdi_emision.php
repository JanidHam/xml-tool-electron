...
class PortalCfdiEmision {
    ...
    function consultar( $directorioAGuardar, $filtros ) {
        try {
            ...
            if( $filtros->folioFiscal != '' ){
                $htmlRespuesta  = $this->consultaEmisorFolio($filtros);
                $nombre         = $filtros->folioFiscal;
            } else {
                $htmlRespuesta  = $this->consultaEmisorFecha($filtros);
                $nombre         = '';
            }
    ...
    function entrarConsultaEmisor($filtros) {
        $url = $this->urlPortalCfdi . 'ConsultaEmisor.aspx';
        $respuesta = $this->sesion->get( $url, ['cookies' => true, 'future' => true, 'verify' => FALSE] );
        ...
        $encabezados = $this->header->obtenerAJAX(
            $this->hostPortalCfdi, 
            $this->urlPortalCfdi . 'ConsultaEmisor.aspx'
        );
    ...
    function consultaEmisorFecha($filtros){
        $url = $this->urlPortalCfdi . 'ConsultaEmisor.aspx';
        $htmlRespuestaArr = $this->entrarConsultaEmisor($filtros);
    ...
    function consultaEmisorFolio($filtros){
        $url        = $this->urlPortalCfdi . 'ConsultaEmisor.aspx';
        $respuesta  = $this->sesion->get($url, ['cookies' => true, 'future' => true, 'verify' => FALSE]);
